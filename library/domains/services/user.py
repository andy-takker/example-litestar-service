...
class UserService:
    ...
    async def fetch_user_by_id(self, *, user_id: UserId) -> User:
        user = await self._user_storage.fetch_user_by_id(user_id=user_id)
        if user is None:
    ...
    async def fetch_user_list(self, *, params: UserPaginationParams) -> UserPagination:
        total = await self._user_storage.count_users(params=params)
        items = await self._user_storage.fetch_user_list(params=params)
        return UserPagination(total=total, items=items)

    async def create_user(self, *, user: CreateUser) -> User:
        return await self._user_storage.create_user(user=user)

    async def delete_user_by_id(self, *, user_id: UserId) -> None:
        if not await self._user_storage.exists_user_by_id(user_id=user_id):
            raise EntityNotFoundException(entity=User, entity_id=user_id)
        await self._user_storage.delete_user_by_id(user_id=user_id)

    async def update_user_by_id(self, *, update_user: UpdateUser) -> User:
        if not await self._user_storage.exists_user_by_id(user_id=update_user.id):
            raise EntityNotFoundException(entity=User, entity_id=update_user.id)
        return await self._user_storage.update_user_by_id(update_user=update_user)
